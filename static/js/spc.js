...
$(function () {
    ...
    $loadBtn.click(function () {
        ...
        if (!selectedProductItem) {
            alert('请选择一个品项（通用CTQ必须手动选择，专属CTQ会自动填充）');
            return;
        }
        var params = {
            ctq_id: ctqId,
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                if (res.message) {
                    $dataWarning.removeClass('d-none').html('<i class="fa fa-info-circle"></i> ' + res.message);
                } else {
                    $dataWarning.addClass('d-none');
                }
                if (res.warning) {
                    $dataWarning.removeClass('d-none').html('<i class="fa fa-exclamation-triangle"></i> ' + res.warning);
                }
                try { drawAllCharts(res); } catch(e) { console.error('绘制控制图失败', e); }
                try { updateCapabilityCards(res); } catch(e) { console.error('更新卡片失败', e); }
                try { renderCapabilityTable(res); } catch(e) { console.error('渲染表格失败', e); }
                try { updateAlarmTable(res); } catch(e) { console.error('更新预警失败', e); }
                try { updateNormality(res); } catch(e) { console.error('正态检验失败', e); }
            },
            error: function (xhr, status, err) {
                if (status === 'timeout') {
                    alert('分析超时：数据量过大，请尝试选择更小的时间范围');
                } else {
                    var errMsg = xhr.responseJSON ? xhr.responseJSON.error : err;
                    alert('加载失败：' + errMsg);
                }
            },
            complete: function () {
    ...
    function drawAllCharts(data) {
        ...
        if (data.usl) {
            xbarSeries.push({ name: 'USL', type: 'line', data: Array(n).fill(data.usl), lineStyle: { type: 'solid', color: '#10b981', width: 1.5 }, symbol: 'none' });
            xbarSeries.push({ name: 'LSL', type: 'line', data: Array(n).fill(data.lsl), lineStyle: { type: 'solid', color: '#10b981', width: 1.5 }, symbol: 'none' });
        }
        if (data.target) {
            xbarSeries.push({ name: '目标', type: 'line', data: Array(n).fill(data.target), lineStyle: { type: 'dashed', color: '#6366f1', width: 1.5 }, symbol: 'none' });
        }
        xbarChart.setOption({
        ...
        window.addEventListener('resize', function () { if (xbarChart && !xbarChart.isDisposed()) xbarChart.resize(); if (rChart && !rChart.isDisposed()) rChart.resize(); if (histChart && !histChart.isDisposed()) histChart.resize(); });
    }
    ...
    var setCap = function(id, value, thresholds) {
        var el = $('#' + id);
        var formatted = (value != null) ? value.toFixed(4) : '-';
        el.text(formatted);
    ...
    function renderCapabilityTable(data) {
        var unit = (data._scale && data._scale.suffix) ? data._scale.suffix : '';
        var html = '<table class="table table-sm table-bordered small">';
        html += '<thead><tr><th colspan="2" class="bg-light">过程能力分析</th></tr></thead>';
        html += '<tbody>';
        html += '<tr><td class="fw-bold">原始样本数</td><td>' + data.all_values.length + '</td></tr>';
        html += '<tr><td class="fw-bold">监测批次数</td><td>' + data.labels.length + '</td></tr>';
        html += '<tr><td class="fw-bold">均值</td><td>' + data.mean.toFixed(4) + unit + '</td></tr>';
        html += '<tr><td class="fw-bold">标准差(组内)</td><td>' + (data.std_within ? data.std_within.toFixed(4) + unit : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">标准差(整体)</td><td>' + data.std_overall.toFixed(6) + unit + '</td></tr>';
        html += '<tr><td class="fw-bold">USL</td><td>' + (data.usl !== undefined ? data.usl.toFixed(4) + unit : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">LSL</td><td>' + (data.lsl !== undefined ? data.lsl.toFixed(4) + unit : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">目标</td><td>' + (data.target !== undefined ? data.target.toFixed(4) + unit : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">Cp</td><td>' + (data.cp != null ? data.cp.toFixed(4) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">Cpk</td><td class="fw-bold ' + (data.cpk >= 1.33 ? 'text-success' : 'text-danger') + '">' + (data.cpk != null ? data.cpk.toFixed(4) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">Pp</td><td>' + (data.pp != null ? data.pp.toFixed(4) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">Ppk</td><td>' + (data.ppk != null ? data.ppk.toFixed(4) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">Cpm</td><td>' + (data.cpm != null ? data.cpm.toFixed(4) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">Cpmk</td><td>' + (data.cpmk != null ? data.cpmk.toFixed(4) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">Z.Bench</td><td>' + (data.z_bench != null ? data.z_bench.toFixed(2) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">PPM 总计</td><td>' + (data.ppm_total != null ? data.ppm_total.toFixed(0) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">PPM > USL</td><td>' + (data.ppm_usl != null ? data.ppm_usl.toFixed(0) : '-') + '</td></tr>';
        html += '<tr><td class="fw-bold">PPM < LSL</td><td>' + (data.ppm_lsl != null ? data.ppm_lsl.toFixed(0) : '-') + '</td></tr>';
        html += '</tbody></table>';
    ...
    function loadCapabilityTrend() {
        ...
        if (!ctqId || !productItem) return;
        var startDate = $startDate.val();
        var endDate = $endDate.val();
        var granularity = $trendGranularity.val();
        $.ajax({
            url: '/spc/capability_trend',
            data: { ctq_id: ctqId, product_item: productItem, start_date: startDate, end_date: endDate, granularity: granularity },
            success: function(res) {
                if (res.error) {
            ...
            error: function() {
                $('#trendChart').html('<div class="alert alert-danger">加载趋势数据失败</div>');
            }
        });
    }
    ...
    function drawTrendChart(data) {
        ...
        var yData = data[metric];
        var targetLine = Array(yData.length).fill(data.target_cpk);
        trendChart.setOption({
            tooltip: { trigger: 'axis' },
            xAxis: { type: 'category', data: data.labels, axisLabel: { rotate: 30, interval: 0 } },
            yAxis: { type: 'value', name: metric.toUpperCase(), min: 0 },
            series: [
                { name: metric.toUpperCase(), type: 'line', data: yData, smooth: false, lineStyle: { width: 2, color: '#6366f1' }, itemStyle: { color: '#6366f1' }, markPoint: { data: [{ type: 'min', name: '最小值' }] } },
                { name: '目标值', type: 'line', data: targetLine, lineStyle: { type: 'dashed', color: '#ef4444' }, symbol: 'none' }
            ]
        });
        window.addEventListener('resize', function() { if (trendChart && !trendChart.isDisposed()) trendChart.resize(); });
    }
