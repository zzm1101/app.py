...
$(function () {
    ...
    function drawAllCharts(data) {
        ...
        if (s > 0) {
            ...
            if (dynamicStep < 0.001) dynamicStep = 0.001;  // 最小步长防止无限循环
    ...
    function loadCapabilityTrend() {
        ...
        if (!ctqId || !productItem) {
            // 清空旧图表并释放引用
            if (trendChart) {
                try { if (!trendChart.isDisposed()) trendChart.dispose(); } catch(e) {}
                trendChart = null;
            }
            $('#trendChart').html('<div class="alert alert-info">请先选择 CTQ 和品项</div>');
            return;
        }
        ...
        $.ajax({
            ...
            data: {
                ctq_id: ctqId, product_item: productItem,
    ...
    // 用于 debounce 的 resize 处理
    var trendResizeTimer = null;
    ...
    function drawTrendChart(data) {
        ...
        if (trendChart) {
            try {
                var containerEl = document.getElementById('trendChart');
                // 仅当实例未销毁且容器内还有 canvas 时才执行 dispose
                if (!trendChart.isDisposed() && containerEl && containerEl.querySelector('canvas')) {
                    trendChart.dispose();
                }
            } catch (e) {
                console.warn('销毁趋势图异常', e);
            }
            trendChart = null;
        }
        ...
        var metric = currentMetric || 'ppk';
        var yData = data[metric];

        // 检查数据有效性
        if (!Array.isArray(yData) || yData.length === 0 || yData.every(function(v) {
            return v === null || isNaN(v);
        })) {
            container.innerHTML = '<div class="alert alert-warning">指标 ' + metric.toUpperCase() + ' 无有效数据，请切换其他指标</div>';
            return;
        }
