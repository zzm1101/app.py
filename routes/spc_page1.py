...
def capability_trend():
    ...
    if start_date: query = query.filter(ProductionData.produce_date >= start_date)
    if end_date: query = query.filter(ProductionData.produce_date <= end_date)
    ...
    if use_boxcox:
        ...
        def boxcox_val(x):
            try: return stats.boxcox(np.array([x]), lmbda=lmbda)[0][0]
            except: return None
        usl, lsl, target = boxcox_val(usl), boxcox_val(lsl), boxcox_val(target)
    ...
    return jsonify({"labels": labels, "ppk": ppk_list, "cpm": cpm_list, "target_cpk": 1.33})
